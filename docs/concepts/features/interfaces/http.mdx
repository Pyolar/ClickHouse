...
<Note>
The `http_column_` parameters only affect `INSERT` queries. For standalone `SELECT` queries they are ignored.
`INSERT ... SELECT` is supported, but requires an explicit column list so ClickHouse knows which
columns the `SELECT` output provides and which come from headers:

```bash
$ curl -sS -H 'X-Event-Type: push' \
    'http://localhost:8123/?query=INSERT+INTO+events+(payload)+SELECT+payload+FROM+src&http_column_X-Event-Type=event_type'
```
